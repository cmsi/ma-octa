...
if [ $VERSION_BASE = "8.3" ]; then
  test -f $OCTADIR.tar.gz || wget -O $OCTADIR.tar.gz 'http://49.212.191.63/phpBB/download/file.php?id=2581&sid=62e37407b95ebf6dbafe6ddfb0c35152'
  mkdir -p $OCTADIR
  ...
  cp -rp INSTALLER83/OCTA8PKG/LICENSE $OCTADIR
  cp -rp INSTALLER83/OCTA8PKG/SCRIPTS $OCTADIR
  tar zxvf INSTALLER83/OCTA8PKG/EXTERNAL_SOFTWARE/jogl-v2.2.4-linux-amd64.tar.gz -C $OCTADIR/GOURMET/lib/linux_64
  tar zxvf INSTALLER83/OCTA8PKG/EXTERNAL_SOFTWARE/jogl-v2.2.4-linux-i586.tar.gz -C $OCTADIR/GOURMET/lib/linux
  rm -rf INSTALLER83
  ...
  (cd $OCTADIR; find . -name Makefile.orig | xargs rm -f)
  (cd $OCTADIR; find . -name *.bak | xargs rm -f)
